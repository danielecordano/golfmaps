...
function setUnit() {
  ...
  if (unit == "yd") radius = 6975220;
  else radius = 6378137;
  if (measuring) handleDrag();
  else setHole(ihole);
}
function toggleUnit() {
  if (selUnit.value == "yd") selUnit.value = "mt";
  else selUnit.value = "yd";
  setUnit();
}
function handleDrag() {
...
function modifyDatabase() {
  const p = holes[ihole].getPath();
  const iholes = ihole.toString();
  let llarray = [];
  for (let i = 0; i < p.length; i++) {
    let ll = { lat: p.getAt(i).lat(), lng: p.getAt(i).lng() };
    llarray[i] = ll;
  }
  const ref = firebase
    .database()
    .ref("courses/" + encName + "/holes/" + iholes);
  ref.set(llarray, function (error) {
    if (error) {
...
async function init() {
  ...
  secondaryColor = getComputedStyle(divMenu).getPropertyValue(
    "--secondary-color"
  );
  ...
  for (let i = 0; i < 18; i++) {
    ...
    p = holes[i].getPath();
    google.maps.event.addListener(p, "set_at", updateDistance);
    google.maps.event.addListener(p, "insert_at", updateDistance);
    google.maps.event.addListener(p, "remove_at", updateDistance);
    google.maps.event.addListener(p, "set_at", modifyDatabase);
    google.maps.event.addListener(p, "insert_at", modifyDatabase);
    google.maps.event.addListener(p, "remove_at", modifyDatabase);
    google.maps.event.addListener(holes[i], "click", (e) => {
  ...
  selHole.addEventListener("change", (e) => {
    setHole(e.target.value - 1);
  });
