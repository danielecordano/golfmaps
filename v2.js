...
function startMeasuring() {
  ...
  holes[ihole].setOptions({ strokeWeight: MINSTROKE, strokeColor: secondaryColor, editable: false });
  if (map.getZoom() < 18)
    map.setZoom(18);
...
function setHole(n) {
  ...
  holes[ihole].setOptions({ strokeWeight: MINSTROKE, strokeColor: secondaryColor, editable: false });
  ihole = n;
  holes[ihole].setOptions({ strokeWeight: MAXSTROKE, strokeColor: primaryColor, editable: true });
  const c = {
...
function modifyDatabase() {
  ...
  const ref = firebase.database().ref('courses/' + encName + '/holes/' + iholes);
